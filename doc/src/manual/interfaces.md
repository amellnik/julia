...
| Methods to implement | Brief description |
|:-------------------- |:----------------- |
| `Broadcast.rule(::Type{SrcType}) = ContainerType` | Default type produced by broadcasting |
...
The actual allocation of the result array is handled by `Broadcast.similar`, analogous to but
separate from `Base.similar`. The argument structure is considerably more complex:

```julia
Broadcast.similar(f, r::BroadcastResult{ContainerType}, As...)
```

`f` is the operation being performed, `ContainerType` is the resulting container type
(e.g., `Array`, `Broadcast.BottomArray`, `Tuple`, etc.).
`eltype(r)` returns the element type, and `indices(r)` the object's indices.
`As...` is the list of input objects. You may not need to use `As...`
unless they help you build the appropriate object; the fallback definition is

```julia
Broadcast.similar(f, r::Broadcast.Result{BottomArray}, As...) = similar(Array{eltype(r)}, indices(r))
```
...
```jldoctest
Broadcast.rule(::Type{AC}) where AC<:ArrayAndChar = AC
```
