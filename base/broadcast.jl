...
indices(A, B...) = broadcast_shape(indices(A), indices(B...))

## Allocating the output container
Base.similar(f, r::Result{BottomArray,ElType}, As...) where ElType = similar(Array{ElType}, indices(r))
...
function broadcast(f, ::Result{ContainerType,Void,Void}, A, Bs...) where ContainerType
    ElType = broadcast_eltype(f, A, Bs...)
    broadcast(f,
              Result{ContainerType,ElType}(indices(A, Bs...)),
              A, Bs...)
end
...
#     function Broadcast.broadcast(f, r::Broadcast.Result{ContainerType,ElType}, As...) where {ContainerType,ElType}
#         ...
#     end
...
function broadcast_nonleaf(f, r::Result{BottomArray,ElType}, As...) where ElType
    nargs = length(As)
    shape = indices(r)
    iter = CartesianRange(shape)
    if isempty(iter)
...
```
"""
broadcast_getindex(src::AbstractArray, I::AbstractArray...) = broadcast_getindex!(Base.similar(Array{eltype(src)}, indices(I...)), src, I...)
